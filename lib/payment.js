...
export class Payment {
  ...
  createUrl(amountDue, orderNumber, timestamp) {
    ...
    if (String(amountDue).includes('.')) {
      amountDue = round(amountDue * 100, 2);
    }

    const hash = this.createPaymentHash(amountDue, orderNumber, timestamp);

    const url = `${this.paymentEndpoint}orderNumber=${orderNumber}&orderType=${
      orderType
    }&amountDue=${amountDue}&paymentMethod=${paymentMethod}&redirectUrl=${
      redirectUrl
    }&redirectUrlParameters=${redirectUrlParameters}&timestamp=${timestamp}&hash=${hash}`;
